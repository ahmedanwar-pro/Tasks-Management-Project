...
type TaskDetailsMobileDueDateCardProps = {
  dueDate: TaskDetailsPopupDetails['dueDate'];
  dueDateValue: TaskDetailsPopupDetails['dueDateValue'];
};

export function TaskDetailsMobileDueDateCard({
  dueDate,
  dueDateValue,
}: TaskDetailsMobileDueDateCardProps): ReactElement {
  const { isFieldPending, saveField } = useTaskDetailsEditing();
  const dueDateButtonRef = useRef<HTMLButtonElement>(null);
  const isDueDatePending = isFieldPending('due_date');

  return (
    <article className="bg-surface-low flex h-20 flex-col gap-1 rounded-md px-4 pt-4 pb-5">
      <div className="flex items-center justify-between gap-3">
        <button
          className="text-text-muted hover:text-text-secondary focus-visible:outline-primary w-fit rounded-xs text-[11px] leading-[16.5px] font-bold uppercase transition-colors focus-visible:outline-2 focus-visible:outline-offset-2"
      ...
      <div className="flex items-center gap-2 pt-1.5">
        <TaskDetailsCalendarIcon className="text-primary" variant="tall" />
        <EditableTaskDueDate
